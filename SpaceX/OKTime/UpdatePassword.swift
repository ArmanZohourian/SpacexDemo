...
enum UpdatePassword: RequestProtocol {
    
case updatePasswordWith(newPassword: String , token: String)
    
    var params: [String : Any] {
        switch self {
        case let .updatePasswordWith(newPassword, token):
            return ["token" : token,
    ...
    var path: String {
        return "/api/v1/users/password/recover"
    }
