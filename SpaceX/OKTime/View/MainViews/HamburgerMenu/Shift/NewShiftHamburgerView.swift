//
//  NewShiftHamburgerView.swift
//  OKTime
//
//  Created by Arman Zohourian on 1/14/23.
//

import SwiftUI

struct NewShiftHamburgerView: View {
        ...
        ActionSheetView {
            ScrollView {
                VStack(alignment: .trailing ,spacing: 10) {
                    ...
                    Button {
                        Task {
                            await getShiftViewModel.addNewShift(startDate: startTime.getFormattedDate(format: "HH:mm:ss"), endDate: endTime.getFormattedDate(format: "HH:mm:ss"), shiftDuration: shiftDuration.getFormattedDate(format: "HH:mm:ss"), shiftTitle: shiftTitle, days: selectedDays, completionHandler: { isSuccessful in
                                if isSuccessful {
                                    isPresneted.toggle()
                        ...
                        GreenFunctionButton(buttonText: "shift_detail_register_shift".localized(language), isAnimated: $getShiftViewModel.isRequesting)
            ...
            .popup(isPresented: $getShiftViewModel.hasError, view: {
                FloatBottomView(colors: colors.redColor, errorMessage: getShiftViewModel.errorMessage)
